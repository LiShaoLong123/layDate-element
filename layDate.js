...
! function() {
    ...
    C.prototype = [], C.prototype.constructor = C, w.extend = function() {
        ...
        show: !1,
        showBottom: !0,
        newBtns:["next","pre","fourteen","clear", "now", "confirm"],
        ...
        if (t.zIndex && (i.style.zIndex = t.zIndex), w.each(new Array(2), function(e) {
                ...
                return "datetime" === t.type && e.push('<span lay-type="datetime" class="laydate-btns-time">' + n.timeTips + "</span>"), w.each(t.btns, function(e, r) {
                    var o = n.tools[r] || "btn";
                    if(t.type==='date'){
                        r ==='next'? o='昨天':r==='fourteen'?o='过去14天':r=='pre'?o='过去七天':"";
                    }else if(t.type==='month'){
                        r ==='next'? o='过去12个月':r==='fourteen'?o='去年':r=='pre'?o='今年':"";
                    }
                    if(r==='fourteen'){b="&nbsp;&nbsp;"}else{b="";}
                    t.range && "now" === r || (a && "clear" === r && (o = "cn" === t.lang ? "重置" : "Reset"), i.push('<span lay-type="' + r + '" class="laydate-btns-' + r + '">' + o + "</span>"+b))
        ...
        var n = this,
            ...
            o = {
                ...
                next:function () {
                    if(a.type==='date'){
                        n.setValue(n.formatTime(-1)).remove();
                    }else if(a.type==='month'){
                        n.setValue(n.formatTime(-365)).remove();
                    }
                },
                fourteen:function () {
                    if(a.type==='date'){
                        n.setValue(n.formatTime(-14)).remove();
                    }else if(a.type==='month'){
                        var dd=new Date();
                        n.setValue((dd.getFullYear()-1)+'-01'+' - '+(dd.getFullYear()-1)+'-12').remove();
                    }
                },
                pre:function () {
                    if(a.type==='date'){
                        n.setValue(n.formatTime(-7)).remove();
                    }else if(a.type==='month'){
                        var dd=new Date();
                        n.setValue(dd.getFullYear()+'-01'+' - '+dd.getFullYear()+'-'+(dd.getMonth()+1)).remove();
                    }
                }
            };
        o[t] && o[t]()
    ...
    }, T.prototype.formatTime=function (difference) {
          var diffTime=difference*24*60*60*1000;
          var now=new Date();
          var lastTime=new Date(now.getTime()+diffTime);
          var date=this.config.type==='date'?'-'+now.getDate():'';
          var date1=this.config.type==='date'?'-'+lastTime.getDate():'';
          var nowStr=now.getFullYear()+'-'+(now.getMonth()+1)+date;
          var lastStr=lastTime.getFullYear()+'-'+(lastTime.getMonth()+1)+date1;
          if(difference>0){
                return nowStr+' - '+lastStr;
